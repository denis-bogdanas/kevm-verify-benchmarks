...
contract SimpleMultiSig8 {
    ...
    constructor(address[] owners_, uint chainId) public {
        require(owners_.length <= 10 && THRESHOLD <= owners_.length);
    ...
    function execute(uint8[8] sigV, bytes32[8] sigR, bytes32[8] sigS, address destination, uint value, bytes data, address executor, uint gasLimit) public {
