...
contract SimpleMultiSig10 {
    ...
    constructor(address[] owners_, uint chainId) public {
        require(owners_.length <= THRESHOLD && THRESHOLD <= owners_.length);
    ...
    function execute(uint8[10] sigV, bytes32[10] sigR, bytes32[10] sigS, address destination, uint value, bytes data, address executor, uint gasLimit) public {
